...
file.isLink = function (target) {
  return (file.exists(target) && fs.lstatSync(target).isSymbolicLink());
};

// True if the path is a directory.
file.isDir = function (target) {
  return (file.exists(target) && fs.statSync(target).isDirectory());
};

// True if the path is a file.
file.isFile = function (target) {
  return (file.exists(target) && fs.statSync(target).isFile());
};
