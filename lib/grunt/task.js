...
// Load tasks and handlers from an array listing locally-installed Npm module(s) 
// which are installed relative to the base dir.
task.loadNpmTasks = function(tasks) {
  var root = path.resolve('node_modules');
    tasks.forEach(function(task) {
      loadTasksMessage('"' + task + '" local Npm module');
      var pkgfile = path.join(root, task, 'package.json');
      ...
      var tasksdir = path.join(root, task, 'tasks');
      if (grunt.file.exists(tasksdir)) {
        loadTasks(tasksdir);
      } else {
        grunt.log.error('Local Npm module "' + task + '" not found. Is it installed?');
      }
  })
};
