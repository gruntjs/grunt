(function(exports) {
  ...
  Task.prototype.task = function(names) {
    // The `names` arg can be a string or an array of strings (or tasks). If
    // multiple string arguments are passed in, handle that.
    if (arguments.length > 1) {
      names = [].slice.call(arguments);
    }
    // If names isn't an array, make it into an array.
