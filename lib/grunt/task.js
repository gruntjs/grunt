...
task.normalizeMultiTaskFiles = function(data, target, taskName) {
  var prop, obj;
  var files = [];
  ...
  // Timestamp of last call with same target and taskName.
  var lastRun;
  ...
  files = grunt.util._(files).chain().forEach(function(obj) {
    ...
    if (expandOptions.newer && target && taskName) {
      if (!lastRun) {
        var stamp = path.join('.grunt', taskName, target, 'timestamp');
        if (!grunt.file.exists(stamp)) {
          lastRun = new Date(0);
        } else {
          lastRun = fs.statSync(stamp).mtime;
        }
        grunt.file.write(stamp, '');
      }
      // In a task config, the newer option can be a boolean or 'any'.
      expandOptions.anyNewer = (expandOptions.newer === 'any' || expandOptions.anyNewer);
      expandOptions.newer = lastRun;
    }
...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    this.files = task.normalizeMultiTaskFiles(this.data, target, name);
