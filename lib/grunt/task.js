...
task.init = function(tasks, nocomplain) {
  ...
  // Load Npm-installed grunt plugin tasks.
  if (grunt._npmTask) { task.loadNpmTasks(grunt._npmTask); }

  // If all specified tasks are init tasks, don't load config.
  var noconfig = tasks.every(function(name) {
    var obj = task._taskPlusArgs(name).task;
    return obj && obj.init;
  });

  // Also don't complain if config should be ignored.
  nocomplain = nocomplain || noconfig;
