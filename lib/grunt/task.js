...
task.registerTask = function(name, info, fn) {
  ...
  thisTask.fn = function(arg) {
    ...
    this.options = function() {
      return grunt.util._.defaults.apply(null, [
        {},
        grunt.config([name, 'options'])
      ].concat(grunt.util.toArray(arguments)));
    };
...
task.registerMultiTask = function(name, info, fn) {
  task.registerTask(name, info, function(target) {
    ...
    if (grunt.util.kindOf(this.data) === 'object') {
    ...
    if (this.file.src) {
      this.file.src = grunt.util.recurse(this.file.src, function(src) {
