...
task.loadNpmTasks = function(name, options) {
    options || ( options = {} );
    loadTasksMessage('"' + name + '" local Npm module');
    if ( typeof options.cwd == 'undefined' ){
        options.cwd = './';
    }
    var root = path.resolve(options.cwd, 'node_modules');
  ...
  if (grunt.file.exists(tasksdir)) {
    loadTasks(tasksdir);
        ...
        var nextDir = grunt.file.findup(path.join('node_modules', name), {
            cwd: path.resolve(path.join('node_modules', name)),
            nocase: true
        });
        nextDir && (nextDir = path.resolve(nextDir, '..', '..'));
        if ( nextDir && nextDir != options.cwd ){
