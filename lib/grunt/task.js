...
exports.registerBasicTask = function(name, info, fn) {
  exports.registerTask(name, info, function(target) {
    ...
    if (!target) {
      return exports.runAllTargets(name);
    }
    // Fail if any required config properties have been omitted.
    config.requires([name, target]);
    // If data is not an object, create an object with src and data properties.
    //   'prop': [srcfiles]
    //   {prop: {src: [srcfiles], dest: 'destfile'}}.
    var data = config([name, target]);
    if (utils.kindOf(data) !== 'object') {
      data = {src: data, dest: target};
    }
    // Process dest as a template.
    if (data.dest) {
      data.dest = template.process(data.dest, config());
    }
    // Expose data object on `this` (as well as task.current).
    this.data = data;
    // Call original task function, passing in the target and any other args.
    return fn.apply(this, [data].concat(utils.toArray(arguments)));
  });
