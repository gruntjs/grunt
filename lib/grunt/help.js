...
Object.keys(grunt.task._tasks).forEach(function(name) {
  ...
  var tasks = tasksByInfo[task.meta.info];
  if (!tasks) { tasks = tasksByInfo[task.meta.info] = []; }
  tasks.push(task);
});
...
opts.forEach(function(opt) {
  grunt.log.writetableln(widths, ['', grunt.utils._.pad(opt[0], col1len), '', opt[1]]);
});
...
Object.keys(tasksByInfo).forEach(function(name) {
  grunt.log.header('Available ' + name + ' tasks');
  tasksByInfo[name].forEach(function(task) {
    var info = task.info;
    if (task.multi) { info += ' *'; }
    grunt.log.writetableln(widths, ['', grunt.utils._.pad(task.name, col1len), '', info]);
  });
});
