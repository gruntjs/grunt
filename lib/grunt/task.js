...
function loadTasks(tasksdir) {
  try {
    var files = require.extensions['.ls'] ? grunt.file.glob.sync('*.{js,coffee,ls}', {cwd: tasksdir, maxDepth: 1}) : grunt.file.glob.sync('*.{js,coffee}', {cwd: tasksdir, maxDepth: 1});
...
task.init = function(tasks, options) {
  ...
  var gruntfile = allInit ? null : grunt.option('gruntfile') ||
    (require.extensions['.ls'] ? grunt.file.findup('Gruntfile.{js,coffee,ls}', {nocase: true}) : grunt.file.findup('Gruntfile.{js,coffee}', {nocase: true}));
