...
var registry = {tasks: [], helpers: [], untasks: [], unhelpers: [], meta: {}};
...
['Task', 'Helper'].forEach(function(mode) {
  var prop = mode.toLowerCase();

  // Override built-in renameTask / renameHelper to use the registry.
  task['rename' + mode] = function(oldname, newname) {
    // Add and remove task/helper.
    registry['un' + prop + 's'].push(oldname);
    registry[prop + 's'].push(newname);
    // Actually rename task/helper.
    return parent['rename' + mode].apply(task, arguments);
  };

  // Override built-in unregisterTask / unregisterHelper to use the registry.
  task['unregister' + mode + 's'] = function() {
    // Add tasks/helpers to "un" registry.
    task.parseArgs(arguments).forEach(function(name) {
      registry['un' + prop + 's'].push(name);
    });
    // Actually unregister tasks/helpers.
    return parent['unregister' + mode + 's'].apply(task, arguments);
  };
});
...
function loadTask(filepath) {
  ...
  // Reset registry.
  registry = {tasks: [], helpers: [], untasks: [], unhelpers: [], meta: {info: lastInfo, filepath: filepath}};
  ...
  try {
    ...
    // Log registered/renamed/unregistered tasks and helpers.
    ['Tasks', 'Helpers'].forEach(function(name) {
      var suffix = name.toLowerCase();
      ['un', ''].forEach(function(prefix) {
        var list = grunt.util._.chain(registry[prefix + suffix]).uniq().sort().value();
        if (list.length > 0) {
          regCount++;
          grunt.verbose.writeln((prefix ? '- ' : '+ ') + name + ': ' +
            grunt.log.wordlist(list));
        }
      });
    });
    if (regCount === 0) {
      grunt.verbose.error('No tasks or helpers were registered or unregistered.');
    }
  } catch(e) {
