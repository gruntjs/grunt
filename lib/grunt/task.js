...
task.normalizeMultiTaskFiles = function(data, target) {
  ...
  // Normalize all .src properties to flattened array.
  files.forEach(function(obj) {
    obj.src = Array.isArray(obj.src) ? grunt.util._.flatten(obj.src) : [obj.src];
  });

  // Process each normalized file object.
  return grunt.util._(files).chain().map(function(obj) {
    ...
    if (obj.expand) {
      return grunt.file.expandMapping(obj.src, obj.dest, expandOptions).map(function(mapObj) {
        ...
        ['expand', 'cwd', 'flatten', 'rename'].forEach(function(prop) {
          delete result[prop];
        });
        return result;
      });
    ...
    if ('src' in result) {
      ...
      Object.defineProperty(result, 'src', {
        ...
        get: function fn() {
          ...
          if (!('result' in fn)) {
            // Process obj.src..
            src = grunt.config.process(obj.src);
    ...
    if ('dest' in result) {
      ...
      result.dest = grunt.config.process(obj.dest);
    }
...
task.registerMultiTask = function(name, info, fn) {
  ...
  task.registerTask(name, info, function(target) {
    ...
    // Target data.
    var data = grunt.config.getRaw([name, target]);
    // Initialize the errorcount for this task.
    errorcount = grunt.fail.errorcount;
    ...
    grunt.util.async.forEachSeries(files, function(fileObj, next) {
      ...
      var src;
      var dest = String(context.file.dest || '[no matches]').cyan;
      var output;
      if ('src' in context.file) {
        src = context.file.src.length > 0 ? grunt.log.wordlist(context.file.src) : '[no matches]'.red;
        if ('dest' in context.file) {
          output = src + ' -> ' + dest;
        } else {
          output = src;
        }
      } else if ('dest' in context.file) {
        output = '[unspecified]'.cyan + ' -> ' + dest;
      }
      if (output) {
        grunt.verbose.writeln('Files: ' + output);
      }
