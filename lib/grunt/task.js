...
var task = grunt.util.task;
var parent = task.create();

// send all task events up
var emitter = grunt.event;
task.emitter.on('*', function (thing) {
  var event = [this.event];
  var args = [event];
  // pass through extra info on te event
  if (thing) {
    if (thing.task.name) {
      args.push(thing.task.name);
    }
    // break out the events by type
    if (thing.args && thing.args.length) {
      event.unshift('task');
      args.push(thing.args[0]);
    } else if (thing.task.alias) {
      event.unshift('alias');
    } else {
      event.unshift('multi');
    }
