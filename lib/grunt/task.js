...
function expandByMethod(method) {
  var patterns = grunt.utils.toArray(arguments).slice(1);
  var filepaths = {};
  ...
  task.searchDirs.forEach(function(dirpath) {
    ...
    var abspatterns = patterns.map(function(pattern) {
      return path.join(dirpath, pattern);
    });
    // Expand the path in case a wildcard was passed.
    grunt.file[method].apply(null, abspatterns).forEach(function(abspath) {
...
task.init = function(tasks, nocomplain) {
  ...
  // Search dirs should be unique.
  task.searchDirs = grunt.utils._.uniq(task.searchDirs);
};
