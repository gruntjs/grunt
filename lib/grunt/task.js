...
exports.init = function(tasks) {
  ...
  tasksdirs.unshift(path.join(__dirname, 'tasks'));
  // Load all built-in and user-specified tasks.
  tasksdirs.forEach(function(tasksdir, i) {
    try {
  ...
  // Are all specified tasks "init" tasks?
  var allInitTasks = tasks.every(function(name) {
  ...
  } else if (allInitTasks) {
    // All tasks are init tasks. Don't complain about missing config file.
