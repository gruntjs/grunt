(function(exports) {
  ...
  Task.prototype._taskPlusArgs = function(name) {
    ...
    return {nameArgs: name, task: task, args: name.split(':').slice(1)};
  };
  ...
  Task.prototype.run = function() {
    ...
    if (fails.length > 0) {
      this._throwIfRunning({name: 'TaskError', message: 'Task "' + fails[0].nameArgs + '" not found.'});
      return this;
    }
      ...
      var complete = function(status, errorObj) {
        ...
        if (status === false && this._options.error) {
          this._options.error.call({name: thing.task.name, nameArgs: thing.nameArgs}, errorObj ||
            {name: 'TaskError', message: 'Task "' + thing.nameArgs + '" failed.'});
        }
        // Run the next task.
        nextTask();
      }.bind(this);
