...
// The name of default template delimiters.
template.defaultDelimitersName = 'config';

// The underscore default template syntax should be a pretty sane default for
// the config system.
template.addDelimiters(template.defaultDelimitersName, '<%', '%>');

// Set Lo-Dash template delimiters.
template.setDelimiters = function(name) {
  // Get the appropriate delimiters.
  var delimiters = allDelimiters[name in allDelimiters ? name : template.defaultDelimitersName];
