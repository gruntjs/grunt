...
// If props is an array, convert it to a props string.
function getPropsString(props) {
  if (util.kindOf(props) === 'array') {
    return props.map(exports.escape).join('.');
  }
  return props;
}
...
exports.requires = function() {
  var props = util.toArray(arguments).map(getPropsString);
  var msg = 'Verifying option' + (props.length === 1 ? '' : 's') +
    ' ' + log.wordlist(props) + ' exist' + (props.length === 1 ? 's' : '') +
    ' in config...';
  verbose.write(msg);
  var initialized = config.initialized();
  var failProps = initialized && props.filter(function(prop) { return !config(prop); }).join(' ');
  if (initialized && !failProps) {
    verbose.ok();
    return true;
  } else {
    ...
    if (!initialized) {
      ...
      throw {name: 'TaskError', message: 'Required config properties "' + failProps + '" missing.'};
    }
  }
};
