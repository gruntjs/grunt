...
file.exists = function (target) {
  return path.existsSync(target);
};

// True if the file is a symbolic link.
file.isLink = function (target) {
  return (path.existsSync(target) && fs.lstatSync(target).isSymbolicLink());
};

// True if the path is a directory.
file.isDir = function (target) {
  return (path.existsSync(target) && fs.statSync(target).isDirectory());
};

// True if the path is a file.
file.isFile = function (target) {
  return (path.existsSync(target) && fs.statSync(target).isFile());
};
