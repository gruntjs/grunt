...
(function(exports) {
  ...
  // Remove one or more previously-registered helpers.
  Task.prototype.unregisterHelpers = function() {
    // Remove helper(s) from cache.
  ...
  Task.prototype.helper = function(isDirective, name) {
    ...
    if (!helper) {
      // Helper not found.
      this._throwIfRunning(new Error('Helper "' + name + '" not found.'));
      return;
    }
  ...
  Task.prototype.getDirectiveParts = function(str) {
    ...
    var parts = this.splitArgs(matches[1]);
  ...
  // the value.
  Task.prototype.directive = function(value, fallback) {
    var directive;

    // Get parts if a string was passed and it's a directive.
    if (typeof value === 'string') {
      directive = this.getDirectiveParts(value);
    }
    ...
    if (directive) {
      ...
      // Not a directive, but a fallback was passed. If it's a callback, pass
      // the value into the callback and return its result, otherwise just
  ...
  // Remove one or more previously-registered tasks.
  Task.prototype.unregisterTasks = function() {
    // Remove task(s) from cache.
    this.parseArgs(arguments).forEach(function(name) {
    ...
    var nextTask = function() {
      ...
      var complete = function(success) {
        ...
        if (success === false) {
          ...
          err = new Error('Task "' + thing.nameArgs + '" failed.');
        } else if ({}.toString.call(success) === '[object Error]') {
        ...
        this._success[thing.nameArgs] = success;
        // If task failed, call error handler.
        if (!success && this._options.error) {
          this._options.error.call({name: thing.task.name, nameArgs: thing.nameArgs}, err);
        }
        // Run the next task.
        nextTask();
      }.bind(this);
      ...
      try {
        ...
        var success = thing.task.fn.apply(this.current, thing.args);
