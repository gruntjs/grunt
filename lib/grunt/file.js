...
exports.expand = function() {
  ...
  var patterns = arguments[0] instanceof Array ? arguments[0] : utils.toArray(arguments);
  // Generate a should-be-unique number.
  var uid = +new Date();
  // Return a flattened, uniqued array of matching file paths.
  return utils._(patterns).chain().flatten().map(function(pattern) {
    // If pattern is a template, process it accordingly.
    pattern = template.process(pattern, config());
...
exports.clearRequireCache = function() {
  ...
  var filepaths = typeof arguments[0] !== 'string' ? arguments[0] : utils.toArray(arguments);
...
exports.npmTaskpath = function(name) {
  var filepath;
  try {
...
exports.taskpaths = function() {
  ...
  taskpaths = taskpaths.concat(optiontasks.map(function(relpath) {
    var npmTaskpath = exports.npmTaskpath(relpath);
    if (npmTaskpath) {
...
exports.taskfileDefaults = function() {
  ...
  if (!result) {
    ...
    if (filepaths.length) {
      ...
      filepaths.forEach(function(filepath) {
        utils._.defaults(result, exports.readJson(filepath));
      });
    }
  }
  return result;
