...
file.expandMapping = function(patterns, destBase, options) {
  ...
  file.expand(options, patterns).forEach(function(src) {
    ...
    if (options.ext) {
      var lastExt = destPath.lastIndexOf('.');
      var lastDir = destPath.lastIndexOf('/');
      // Check for last extension
      if (lastExt !== -1 && lastExt > lastDir) {
        // Remove extension and add
        destPath = destPath.substr(0, lastExt) + options.ext;
      } else {
        // just add
        destPath = destPath + options.ext;
      }
    }
