...
file.expandToUrls = function() {
  ...
  return file.expand(patterns).map(function(filepath) {
...
file.npmTaskpath = function(name) {
  var filepath;
  try {
...
file.taskpaths = function() {
  var args;
  var taskpaths = [];
  // Path to user's "tasks" files.
  taskpaths.push(file.userpath('tasks'));
  ...
  taskpaths = taskpaths.concat(optiontasks.map(function(relpath) {
    var npmTaskpath = file.npmTaskpath(relpath);
    if (npmTaskpath) {
...
file.taskfiles = function() {
  ...
  file.taskpaths().reverse().map(function(dirpath) {
    var abspath = path.join(dirpath, filepath);
    // Expand the path in case a wildcard was passed.
    file.expand(abspath).forEach(function(abspath) {
...
// data object.
var taskfileReadJSON = {};
file.taskfileReadJSON = function() {
  var filepath = path.join.apply(path, arguments);
  var result = taskfileReadJSON[filepath];
  var filepaths;
  if (!result) {
    result = taskfileReadJSON[filepath] = {};
