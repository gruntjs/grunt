...
task.normalizeMultiTaskFiles = function(data, target) {
  ...
  // Normalize to array and flatten all .src properties.
  ...
  return files.map(function(obj) {
    var result = {};
    var options = {};
    // Copy keys over to result and options objects.
    Object.keys(obj).forEach(function(key) {
      result[key] = obj[key];
      // Blacklist a few properties that minimatch won't care about.
      if (['src', 'dest'].indexOf(key) === -1) {
        options[key] = obj[key];
      }
    });
    // Expose the original (normalized) file obj as .orig.
    result.orig = obj;
      ...
      Object.defineProperty(result, 'src', {
        ...
        get: function fn() {
          ...
          if (!('result' in fn)) {
            ...
            // Expand src files, memoizing result. Pass options into minimatch.
            fn.result = grunt.file.expand(options, src);
          }
          return fn.result;
        }
      });
