...
task.registerTask('lint', 'Validate files with JSHint.*', function(name) {
  if (!name) {
    // An argument wasn't passed. Run this task once for each lint sub-prop.
    return task.runAllProps('lint');
  }

  // Any name with a possible "." has to be escaped!
  var propname = 'lint.' + config.escape(name);
  ...
  file.expand(config(propname)).forEach(function(filepath) {
