...
(function(exports) {
  ...
  var emitter = exports.emitter = new EventEmitter2({ wildcard: true });
  ...
  Task.prototype.runTaskFn = function(context, fn, done) {
    ...
    var complete = function(success) {
      ...
      if (!success && this._options.error) {
        ...
        emitter.emit('fail', thing);
      } else {
        emitter.emit('done', thing);
      }
      done(err, success);
    }.bind(this);
  ...
  Task.prototype.start = function() {
    ...
    var nextTask = function() {
      ...
      // trigger an event marking start of a task/multi-task run
      emitter.emit('start', thing);
