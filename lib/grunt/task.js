...
exports.create = function(grunt) {
  ...
  var task = Object.create(parent);

  // Emit all logging as events.
  task.event = new EventEmitter();
  var log = logMethodsToEvents(task.event);
  ...
  task.registerTask = function(name) {
    ...
    thisTask.meta = _.clone(registry.meta);
    ...
    thisTask.fn = function(arg) {
      ...
      this.options = function() {
        ...
        var options = _.extend.apply(null, args);
      ...
      var logger = _fn.alias || (thisTask.multi && (!arg || arg === '*')) ? log.verbose : log;
      // Actually log.
      ...
      logger.debug('Task source: ' + thisTask.meta.filepath);
  ...
  task.normalizeMultiTaskFiles = function(data, target) {
    ...
    if (util.kindOf(data) === 'object') {
      if ('src' in data || 'dest' in data) {
        ...
        _.flatten(data.files).forEach(function(obj) {
          var prop;
    ...
    if (files.length === 0) {
      log.verbose.writeln('File: ' + '[no files]'.yellow);
      return [];
    }
    ...
    if (grunt.option('verbose')) {
      files.forEach(function(obj) {
        var output = [];
        if ('src' in obj) {
          output.push(obj.src.length > 0 ? log.wordlist(obj.src) : '[no src]'.yellow);
        }
        if ('dest' in obj) {
          output.push('-> ' + (obj.dest ? String(obj.dest).cyan : '[no dest]'.yellow));
        }
        if (output.length > 0) {
          log.verbose.writeln('Files: ' + output.join(' '));
        }
      });
    }
  ...
  task.registerMultiTask = function(name, info, fn) {
    ...
    task.registerTask(name, info, function(target) {
      ...
      Object.defineProperty(this, 'filesSrc', {
        enumerable: true,
        get: function() {
          return _(this.files).chain().pluck('src').flatten().uniq().value();
        }.bind(this)
      });
  ...
  function loadTask(filepath) {
    ...
    try {
      ...
      log.verbose.write(msg).ok();
      // Log registered/renamed/unregistered tasks.
      ['un', ''].forEach(function(prefix) {
        var list = _.chain(registry[prefix + 'tasks']).uniq().sort().value();
        if (list.length > 0) {
          regCount++;
          log.verbose.writeln((prefix ? '- ' : '+ ') + log.wordlist(list));
        }
      });
      if (regCount === 0) {
        log.verbose.warn('No tasks were registered or unregistered.');
      }
    } catch (e) {
      // Something went wrong.
      log.write(msg).error().verbose.error(e.stack).or.error(e);
    }
    // Restore registry.
  ...
  function loadTasksMessage(info) {
    ...
    log.verbose.subhead('Registering ' + info + ' tasks.');
  }
  ...
  function loadTasks(tasksdir) {
    try {
      ...
      log.verbose.error(e.stack).or.error(e);
    }
  }
  ...
  task.loadNpmTasks = function(name) {
    ...
    if (grunt.file.exists(tasksdir)) {
      ...
      log.error('Local Npm module "' + name + '" not found. Is it installed?');
    }
  };
  ...
  task.init = function(tasks, options) {
    ...
    if (options.gruntfile === false) {
      ...
      log.verbose.writeln().write(msg).ok();
