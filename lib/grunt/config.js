...
function getPropString(prop) {
  if (grunt.util.kindOf(prop) === 'array') {
...
function processDirectives() {
  ...
  data = grunt.util.recurse(data, function(value) {
...
config.get = function(prop) {
  ...
  if (prop) {
    ...
    return grunt.util.namespace.get(data, prop);
  } else {
    // Nothing was passed. Return a shallow clone of the actual config data.
    return grunt.util._.clone(data);
  }
};
...
config.set = function(prop, value) {
  ...
  var result = grunt.util.namespace.set(data, prop, value);
  // Process directives.
  processDirectives();
  // Return result.
  return result;
...
config.process = function(prop) {
  return grunt.util.recurse(config.get(prop), function(value) {
    if (typeof value !== 'string') { return value; }
