(function(exports) {
  ...
  Task.prototype.helper = function(name) {
    ...
    if (!helper) {
      // Helper not found.
      this._throwIfRunning({name: 'HelperError', message: 'Helper "' + name + '" not found.'});
    } else {
  ...
  Task.prototype._taskPlusArgs = function(name) {
    // The name (to be iteratively shortened).
    var tmp = name;
    // The task.
    var task;
    do {
      task = this._tasks[tmp];
    } while (!task && (tmp = tmp.replace(/:?[^:]+$/, '')));
    // The task to run and the args to run it with.
    return {name: name, task: task, args: name.split(':').slice(1)};
  };
  ...
  Task.prototype.run = function() {
    ...
    if (fails.length > 0) {
      this._throwIfRunning({name: 'TaskError', message: 'Task "' + fails[0].name + '" not found.'});
      return this;
    }
    ...
    var nextTask = function() {
      ...
      var complete = function(status, errorObj) {
        ...
        if (status === false && this._options.error) {
          this._options.error.call({name: task.name}, errorObj ||
            {name: 'TaskError', message: 'Task "' + task.name + '" failed.'});
        }
        // Run the next task.
        nextTask();
      }.bind(this);
