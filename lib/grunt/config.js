...
exports.create = function(grunt) {
  ...
  var log = logMethodsToEvents(config.event);
  ...
  config.init = function(obj) {
    log.verbose.write('Initializing config...').ok();
  ...
  config.requires = function() {
    ...
    var props = util.toArray(arguments).map(config.getPropString);
    var msg = 'Verifying propert' + p(props.length, 'y/ies') +
      ' ' + log.wordlist(props) + ' exist' + p(props.length, 's') +
      ' in config...';
