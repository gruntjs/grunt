...
task.registerTask = function(name, info, fn) {
  ...
  thisTask.fn = function(arg) {
    ...
    this.extraspaths = grunt.file.taskDirs.bind(grunt.file, this.name);
...
task.loadTasks = function(tasksdir) {
  if (path.existsSync(tasksdir)) {
    grunt.option('tasks').push(tasksdir);
...
task.loadNpmTasks = function(name) {
  var taskspath = grunt.file.npmTaskDir(name);
  if (taskspath) {
    grunt.option('tasks').push(taskspath);
...
task.init = function(tasks, nocomplain) {
  var taskpaths = grunt.file.taskDirs().reverse();
  ...
  userTasks.filter(function(tasksdir) {
    // Exclude tasks in the userDir. Tasks should never be loaded from there,
    // they should be in a plugin or committed with a project.
    return tasksdir !== grunt.file.userDir('tasks');
  }).forEach(function(tasksdir) {
    console.log(tasksdir.green);
    loadTasks(tasksdir, 'user-specified');
  });
};
